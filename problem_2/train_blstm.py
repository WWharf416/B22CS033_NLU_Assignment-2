...
class Seq2SeqBLSTM(nn.Module):
    ...
    def forward(self, x, y=None):
        ...
        _, (h_n, c_n) = self.encoder(embedded_x)
        ...
        out, _ = self.decoder(embedded_y, hidden)
        logits = self.fc(out)
        return logits
...
print(f"Training BLSTM on {device} with Early Stopping (Patience: {PATIENCE})...")
...
for epoch in range(MAX_EPOCHS):
    ...
    for name in train_names:
        ...
        y_in = torch.tensor(seq[:-1], dtype=torch.long).unsqueeze(0).to(device)
        y_target = torch.tensor(seq[1:], dtype=torch.long).unsqueeze(0).to(device)
